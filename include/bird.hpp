...
///
/// @details    This file contains the definition of the Bird class, the Boid class and the Predator class; the last two publicly inherit 
///             from Bird. A Bird object represents a bird flying in a two-dimensional space. Each bird is characterized by
///             two point::Point objects, representing its position and its velocity.
#ifndef BIRD_HPP
#define BIRD_HPP
...
class Bird {
 ...
 public:
  ...
  /// @details Whenever a bird sees other birds near it, an additional component of velocity is evaluated in order to keep it
...
class Boid final : public Bird {
 public:
  ...
  /// @param pos Is the position of the Boid object.
  /// @param vel Is the velocity of the Boid object.
  Boid(point::Point const &pos, point::Point const &vel);

  /// @brief Updates the speed of the Boid object by imposing a maximum speed.
  /// @details If the speed of the Boid object exceeds the maximum allowed, its module is normalized, leaving the
  /// direction unchanged.
  ...
  /// @brief Evaluate the correction to the velocity of the Boid object, in order to keep it aligned with the neighbours.
  /// @details Whenever a Boid object sees other Boid objects near it, an additional component of velocity is evaluated in order to keep it
  /// aligned to the near birds. In particular, the increment depends on the average of the velocities of the
  /// neighbours, from which the velocity of the Boid object itself is subtracted.
  /// @param a Factor which modules the correction.
  /// @param near_boids Identifies the Boid object's neighbours.
  /// @return The correction to the velocity.
  [[nodiscard]] point::Point alignment(double a, const std::vector<std::shared_ptr<Bird>> &near_boids) const;

  /// @brief Evaluate the correction to the velocity of the Boid object, in order to keep a more cohesive unit of Boid objects.
  /// @details Whenever a Boid object sees other Boid objects near it, an additional component of velocity is evaluated in order to make them a
  /// cohesive group. In particular, the increment depends on the average of the positions of the neighbours, from
  /// which the velocity of the Boid object itself is subtracted.
  ...
  /// @brief Evaluate the correction to the velocity of the Boid object, in order to keep it separated from predators.
  /// @details Whenever a Boid object sees Predator objects near it, an additional component of velocity is evaluated in order to keep it
  /// away from the Predator objects and prevent it from being caught.  In particular, the increment depends on the average of
  /// the differences in positions between the Boid object and the near Predator objects.
  /// @param r Factor which modules the repulsion.
  /// @param near_predators Identifies Predator objects near the Boid object.
  /// @return The correction to the velocity.
...
class Predator final : public Bird {
 public:
  ...
  /// @details If the speed of the Predator object exceeds the maximum allowed, its module is normalized, leaving the
  /// direction unchanged.
  ...
  /// @details If the speed of the Predator object exceeds the minimum allowed, its module would be normalized, leaving the
  /// direction unchanged.
  /// @param p_min_speed is the minimum speed for Predator objects.
  /// @param velocity Is the velocity to update.
  void boost(double p_min_speed, point::Point &velocity) override;

  /// @brief Evaluate the correction to the velocity of the Predator object, in order to chase the near boids.
  /// @details Whenever a Predator object sees Boid objects near it, an additional component of velocity is evaluated so that the Predator object
  /// begin to chase them. In particular, the increment depends on the average of the differences in positions
  /// between the Predator object and the near Boid objects.
  /// @param c Factor which modules the correction.
  /// @param near_boids Identifies the Boid objects near the Predator object.
  /// @return The correction to the velocity.
