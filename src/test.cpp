...
TEST_CASE("Testing Boid class") {
  ...
  Point pos2 = pos1 + 0.8 * d * Point(std::sin( M_PI/3. + vel1.angle()), -std::cos( M_PI/3. + vel1.angle()));
  Point vel2(-8.5, -6.);

  Point pos3 = pos1 + 0.8 * d * Point(std::sin(-1 * M_PI/3. + vel1.angle()), -std::cos( -1 * M_PI/3. + vel1.angle()));
  Point vel3(-7.3, 2.5);

  Point pos4 = pos1 + 0.5 * ds * Point(std::sin(M_PI/3. + vel1.angle()), -std::cos(M_PI/3. + vel1.angle()));
  Point vel4(-1., -2.);

  Point pos5 = pos1 + 0.5 * ds * Point(std::sin(-1 * M_PI/3. + vel1.angle()), -std::cos( -1 * M_PI/3. + vel1.angle()));
  Point vel5(-5., -3.2);
...
//  b5.set_velocity(vel5);
  ...
  SUBCASE("Testing separation method") {
    double sep1_x = -s * 0.5 * ds * (std::sin(M_PI/3. + vel1.angle()) + std::sin(-1 * M_PI/3. + vel1.angle()));
    double sep1_y = -s * 0.5 * ds * -(std::cos(M_PI/3. + vel1.angle()) + std::cos(-1 * M_PI/3. + vel1.angle()));
  ...
  SUBCASE("Testing cohesion method") {
    double coh1_x = c * ((4* x +( 0.5 * ds + 0.8 * d) * (std::sin(M_PI/3. + vel1.angle()) + std::sin(-1 * M_PI/3. + vel1.angle())) )/4. - x);
    double coh1_y = c * ((4* y +( 0.5 * ds + 0.8 * d) * -(std::cos(M_PI/3. + vel1.angle()) + std::cos(-1 * M_PI/3. + vel1.angle())) )/4. - y);
  ...
  SUBCASE("Testing repel method") {
    ...
    double rep1_x = -s * 6 * (0.5 * ds + 0.8 * d ) * (std::sin(M_PI/3. + vel1.angle()) + std::sin(-1 * M_PI/3. + vel1.angle()));
    double rep1_y = -s * 6 * (0.5 * ds + 0.8 * d ) * -(std::cos(M_PI/3. + vel1.angle()) + std::cos(-1 * M_PI/3. + vel1.angle()));
  ...
  //TODO: implementare test per border()
 };
...
TEST_CASE("Testing Predator class") {
      ...
      Point pos2 = pos1 + 0.8 * d * Point(std::sin( M_PI/3. + vel1.angle()), -std::cos( M_PI/3. + vel1.angle()));
      Point vel2(-8.5, -6.);
      p2.set_bird(pos2, vel2);

      Point pos3 = pos1 + 0.8 * d * Point(std::sin(-1 * M_PI/3. + vel1.angle()), -std::cos( -1 * M_PI/3. + vel1.angle()));
      Point vel3(-7.3, 2.5);
      p3.set_bird(pos3, vel3);
      ...
      SUBCASE("Testing chase method"){
        //nel test applichiamo la funzione chase su near_p1, i predatori vicini a p1, anche se sarebbe pensata per near_boids
        double chase1_x = c * 2 * ((0.8 * d * (std::sin(M_PI/3. + vel1.angle()) + std::sin(-1 * M_PI/3. + vel1.angle())) /2.));
        double chase1_y = c * 2 * ((0.8 * d * -(std::cos(M_PI/3. + vel1.angle()) + std::cos(-1 * M_PI/3. + vel1.angle())) /2.));
